package com.company;

public class BinarySearchTree implements NodeList{

    private ItemStored root = null;

    public BinarySearchTree(ItemStored root) {
        this.root = root;
    }

    @Override
    public ItemStored getRoot() {
        return this.root;
    }

    @Override
    public boolean addItem(ItemStored item) {
        if(this.root == null){
            this.root = item;
            return true;
        }

        ItemStored currentItem = this.root;
        while(currentItem != null){
            int comparison = currentItem.comparedTo(item);
            if(comparison<0){
                if(currentItem.next() != null){
                    currentItem = currentItem.next();
                } else {
                    currentItem.setNext(item);
                    return true;
                }
            } else if(comparison > 0){
                if(currentItem.previous() != null){
                    currentItem = currentItem.previousItem;
                } else {
                    currentItem.setPrevious(item);
                    return true;
                }
            } else {
                System.out.println(item.getValue() + " is already present");
                return false;
            }
        }
        return false;
    }

    @Override
    public boolean removeItem(ItemStored item) {
        if(item != null){
            System.out.println("Deleting item "+item.getValue());
        }
        ItemStored currentItem = this.root;
        ItemStored parentItem = currentItem;

        while (currentItem != null){
            int comparison = currentItem.comparedTo(item);
            if(comparison<0){
                parentItem = currentItem;
                currentItem = currentItem.next();
            } else if (comparison> 0){
                parentItem = currentItem;
                currentItem = currentItem.previous();
            } else {
                removingItem(currentItem,parentItem);
                return true;
            }
        }
        return false;
    }

    private void removingItem(ItemStored item, ItemStored parent){
        if(item.next() == null){
            if(parent.next() == item){
                parent.setNext(item.previous());
            } else if(parent.previous() == item){
                parent.setPrevious(item.previous());
            } else {
                this.root = item.previous();
            }
        } else if(item.previous() == null){
            if(parent.next() == item){
                parent.setNext(item.next());
            } else if(parent.previous() == item){
                parent.setPrevious(item.next())
            } else{
                this.root = item.next();
            }
        } else {

            ItemStored current = item.next();
            ItemStored leftmostParent = item;
            while (current.previous() != null){
                leftmostParent = current;
                current = current.previous();
            }

            item.setValue(current.getValue());

            if(leftmostParent == item){
                item.setNext(current.next());
            }else {
                leftmostParent.setNext(current.next());
            }

        }


    }

    @Override
    public void traverse(ItemStored root) {

        if(root != null){
            traverse(root.previous());
            System.out.println(root.getValue());
            traverse(root.next());
        }
    }
}
